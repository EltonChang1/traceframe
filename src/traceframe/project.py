...
def init_project(path: str | Path = ".", project_name: str = "traceframe") -> Path:
    ...
    if project_path.exists():
        ...
        metadata["project_name"] = project_name or metadata.get("project_name", "traceframe")
...
def start(project_name: str) -> Path:
...
def get_project_root(path: str | Path = ".") -> Path:
    global _active_project_root
    if _active_project_root is not None and (_active_project_root / TRACEFRAME_DIR).exists():
    ...
    for candidate in [current, *current.parents]:
        if (candidate / TRACEFRAME_DIR).exists():
            ...
            return candidate
    raise TraceFrameProjectError("No .traceframe directory found. Run traceframe init or tf.start(...).")
...
def write_project_metadata(project_name: str, path: str | Path = ".") -> None:
    ...
    metadata.update({"project_name": project_name, "traceframe_version": TRACEFRAME_VERSION})
